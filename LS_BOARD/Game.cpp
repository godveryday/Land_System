...
void Game::PrintCurrent()
{
    ...
    cout << "현재 모드 : ";
    if (moveMode) cout << "이동";
    else if (attackMode) cout << "공격";
    else if (spawnMode) cout << "생성";
    else cout << "대기";
    cout << endl;

    cout << "선택 : ";
    if (isSelected) cout << "선택 O";
    else cout << "선택 X";
    cout << endl;
...
void Game::Check_KeyInput()
{
    ...
    case 3:  // ctrl+c
        if (playerTurn == 0)
            cout << "RED가 항복했습니다" << endl;
        else
            cout << "BLUE가 항복했습니다" << endl;
        exit(0);
        break;
...
void Game::handleSpaceKey()
{
    ...
    else
    {
        ...
        { 
            ...
            // owner를 확인, owner에 따라, playerName, player를 정의 (이후 매개변수로 들어감)
            ...
            if (check_spawnable)
            {
                ...
                if (command && board[CursorY][CursorX] == nullptr)
                {
                    ...
                    if (has_empty_slot(player))
                    {
                        ...
                        {
                            cout << "이미 생성 대기 중인 객체가 있습니다" << endl;;
                        }
                    }
                    else
                    {
                        cout << "생성할 수 있는 최대 유닛 갯수는 10개입니다" << endl;
                    }
                }
                else
                {
                    cout << "해당 위치에 이미 객체가 존재합니다" << endl;;
                }
            }
            else
            {
